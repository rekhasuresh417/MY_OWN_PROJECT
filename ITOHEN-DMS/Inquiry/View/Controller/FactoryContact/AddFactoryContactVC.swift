...
class AddFactoryContactVC: UIViewController {
    ...
    var factoriesDelegate: AddFactoryContactsDelegate?
    ...
    func setupUI(){
        ...
        self.setup(suppliersTextField, placeholderLabel: "\(LocalizationManager.shared.localizedString(key: "factoryText")) *", color: .inquiryPrimaryColor())
        self.setup(contactPersonTextField, placeholderLabel: "\(LocalizationManager.shared.localizedString(key: "contactPersonText")) *", color: .inquiryPrimaryColor())
        self.setup(contactNumberTextField, placeholderLabel: "\(LocalizationManager.shared.localizedString(key: "contactNumberText")) *", color: .inquiryPrimaryColor())
        self.setup(emailTextField, placeholderLabel: "\(LocalizationManager.shared.localizedString(key: "emailText")) *", color: .inquiryPrimaryColor())
        self.setup(addressTextField, placeholderLabel: "\(LocalizationManager.shared.localizedString(key: "addressText")) *", color: .inquiryPrimaryColor())
        self.setup(cityTextField, placeholderLabel: "\(LocalizationManager.shared.localizedString(key: "cityText")) *", color: .inquiryPrimaryColor())
    ...
    @objc func saveButtonTapped(_ sender: UIButton){
        ...
        let params:[String:Any] = [
            "factory" : self.suppliersTextField.text ?? "",
        ...
        print(params)
        self.showHud()
        RestCloudService.shared.addNewInquiryFactory(params: params)
    }
    ...
    private func inputFieldsValidation() -> Bool{
        ...
        if (self.suppliersTextField.text ?? "").isEmptyOrWhitespace(){
            message = "\(LocalizationManager.shared.localizedString(key: "pleaseEnterText")) \(LocalizationManager.shared.localizedString(key: "factoryText"))"
...
extension AddFactoryContactVC: UITextFieldDelegate{
...
extension AddFactoryContactVC: RCFactoryContactDelegate{
    
    /// Save Inquiry Factory
    func didReceiveSaveInquiryFactory(message: String){
        self.hideHud()
        UIAlertController.showAlertWithCompletionHandler(message: String().getAlertSuccess(message: message), target: self, alertCompletionHandler: { _ in
            DispatchQueue.main.async {
                self.factoriesDelegate?.AddFactoryContactsList()
                self.dismissViewController()
            }
        })
    }
    func didFailedToReceiveSaveInquiryFactory(errorMessage: String){
        self.hideHud()
